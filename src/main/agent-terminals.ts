...
export class AgentTerminalManager {
  ...
  create(params: Record<string, any>, fallbackCwd: string): { terminalId: string } {
    ...
    // Grok currently sends shell-form commands such as `/bin/bash -lc pwd`
    // without an `args` field. ACP also permits executable + args clients, so
    // retain direct spawning when that field is explicitly present.
