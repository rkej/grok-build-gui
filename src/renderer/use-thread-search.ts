...
export function useThreadSearch(timelinePaneRef: RefObject<HTMLDivElement | null>) {
  ...
  const clearMarks = useCallback(() => {
    ...
    if (!pane) return;
    pane.querySelectorAll("mark.thread-find-match, mark.thread-find-active").forEach((mark) => {
  ...
  const searchImmediate = useCallback((q: string) => {
    ...
    while ((node = walker.nextNode() as Text | null)) {
      if (node.textContent && node.textContent.toLowerCase().includes(lowerQuery)) textNodes.push(node);
    }
  ...
  const goToMatch = useCallback((direction: 1 | -1) => {
    ...
    if (current) current.className = "thread-find-match";
